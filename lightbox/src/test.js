...
const MyComponent = () => {
            ...
            window.PayWithConverge.open(
                { ssl_txn_auth_token: sessionToken },
                {
                    ...
                    onApproval: handleResponse('approval', response => {
                        setResponse({ type: 'approval', data: JSON.stringify(response, null, '\t'), successData: response });
                    }),
                }
            );
        } else {
    ...
    const handleResponse = (type, dataCallback) => (response) => {
        ...
        setResponse({ type, data: dataCallback ? dataCallback(response) : '', successData: type === 'approval' ? response : null });
    };
